/**
 * Deterministic differential corpus (Phase 1.3): every (threshold, shareCount)
 * pair with 1 ≤ t ≤ n ≤ 16, every even secret length in [16, 32], four
 * seeds; recovery from subsets (up to size 5) of a 2-of-5 and a 3-of-7
 * split, with bit-flip corruptions; every invalid-parameter combination in
 * validator order. Pure and deterministic.
 */
import type { Recipe, RngSpec, SplitSpec, Bytes } from "../vectors/recipes";
...
function* recovers(): Generator<Recipe> {
  ...
  for (const from of bases) {
    if (from.n === 16) {
      ...
      for (const indexes of subsets(from.n, 5)) yield { k: "recover", from, indexes };
    }
    // corruptions of a valid threshold subset
    const idx = Array.from({ length: from.t }, (_, i) => i);
    for (const byte of [0, 1, 3, 4, 7, 15]) {
      for (const mask of [0x01, 0x80]) {
        yield { k: "recover", from, indexes: idx, corrupt: { share: 0, byte, mask } };
        yield { k: "recover", from, indexes: idx, corrupt: { share: from.t - 1, byte, mask } };
      }
    }
  }
...
export const categories: Record<string, () => Generator<Recipe>> = { splits, recovers, invalid };
