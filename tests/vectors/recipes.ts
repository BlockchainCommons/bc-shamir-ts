 ...
 * `materialize` runs it through a `VectorApi` and returns one outcome string,
...
export type Recipe =
  | ({ k: "split" } & SplitSpec)
  | { k: "recover"; shares: { index: number; data: Bytes }[] }
  | { k: "recover"; from: SplitSpec; indexes: number[]; corrupt?: Corruption };
...
export function materialize(api: VectorApi, r: Recipe): Outcome {
  try {
    if (r.k === "split") {
      return api.split(r.t, r.n, toBytes(r.secret), api.makeRng(r.rng)).map(hex).join(",");
    }
    let indexes: number[];
    let shares: Uint8Array[];
    if ("shares" in r) {
      indexes = r.shares.map((s) => s.index);
      shares = r.shares.map((s) => toBytes(s.data));
    } else {
      const all = api.split(r.from.t, r.from.n, toBytes(r.from.secret), api.makeRng(r.from.rng));
      indexes = r.indexes;
